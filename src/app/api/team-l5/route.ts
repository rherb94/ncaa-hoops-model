...
//
// GET /api/team-l5?teamId=INTERNAL_TEAM_ID
...
export async function GET(req: NextRequest) {
  ...
  const schedUrl =
    ...
    `/mens-college-basketball/teams/${espnId}/schedule`;
  ...
  const games: L5Game[] = last5.map((ev: any) => {
    const comp = ev.competitions[0];
    // Match by team.id (string comparison to handle int/string mismatch)
